...
for i = 2:N
    if i ~= 1
        trueStates(:,i) = stateModel(state_vector(:,i-1),Ts) +sqrt(ekf.ProcessNoise)*randn(6,1);  
    end
...
plot(Tsec(2:end)./(3600),estimateStates(3,2:end),"b--",'LineWidth',4,DisplayName="speed est")
